...
switch(_side) do
{
	...
	case independent: {
		_query = format["UPDATE players SET cash='%1', bankacc='%2',medic_gear='%3' WHERE playerid='%4'",_cash,_bank,_gear,_uid];
	};
	
	case east: {
		_query = format["UPDATE players SET cash='%1', bankacc='%2', asadac_gear='%3' WHERE playerid='%4'",_cash,_bank,_gear,_uid];
	};
};
