...
if ((life_cashCache != (life_cash / 2) + 5) || (life_atmCashCache != (life_atmCash / 2) + 3)) exitWith
{
	[] spawn 
	{
		[[1, format["The Asylum server has detected %1 (ID %2) hacking money. Please report this to an administrator.", player getVariable["realname",name player], getPlayerUID player]],"life_fnc_broadcast",nil,false] spawn BIS_fnc_MP;
		sleep 3;
		endMission "Loser";
	};
};
