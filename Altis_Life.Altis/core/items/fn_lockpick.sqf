...
_dice = random(100);	// alarm
if(_dice < 31) then {
	[_curTarget,"car_alarm"] call life_fnc_globalSound;
};
_dice = 0; 				//arlarm
...
if(!_isVehicle) then {
	...
	if(_dice < 30) then {
		...
		[[0,format["%1 hat versucht ein Auto aufzubrechen.",player getVariable["realname",name player]]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
