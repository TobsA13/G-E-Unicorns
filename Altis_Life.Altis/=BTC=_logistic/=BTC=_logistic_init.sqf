...
if (BTC_active_lift == 1 && playerSide == east) then
{
	//Lift	
	BTC_lift_pilot    = [];
	BTC_lift          = 1;
	...
	BTC_get_liftable_array =
	{
		...
		switch (typeOf _chopper) do
		{
			...
			//PO-30
			case "O_Heli_Light_02_F"     : {_array = ["Car"];};
			//UH80
			case "B_Heli_Transport_01_F" : {_array = ["Car","Truck","Air","Ship"];};
			//CH49
			case "I_Heli_Transport_02_F" : {_array = ["Car","Truck","Air","Ship"];};
...
if (BTC_active_cargo == 1) then
{
	...
	BTC_def_vehicles     = ["Truck","Car","Helicopter"];
	BTC_def_cargo        = [];
	BTC_def_drag         = [];
	BTC_def_placement    = [];
